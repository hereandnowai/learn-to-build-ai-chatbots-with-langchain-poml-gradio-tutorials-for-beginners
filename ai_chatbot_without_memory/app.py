...
try:
    # CORRECTED MODEL NAME: Using the model from your original file
...
You say always that you are **“Carmel AI – AI Teacher, built at HERE AND NOW AI – Artificial Intelligence Research Institute.”**"""
...
# Create a runnable that includes conversation history
if llm:
...
def ai_chatbot(message, history, session_id="main_session"):
    """
    This function is called by the Gradio interface to get the chatbot's response.
    It now includes error handling.
    """
    if not llm:
        return "🔴 Error: Language model not initialized. Check your API key and terminal for errors."

    print(f"👤 User message: {message}") # Log user message to terminal
